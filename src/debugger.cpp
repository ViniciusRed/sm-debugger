...
#define DISP_MASK 0x0f
    ...
    int
        get_symbolvalue(const sp::Symbol* sym, int index, cell_t* value)
    {
        cell_t* vptr;
        cell_t base = sym->addr();
...
#define MULTIPLIER 1000
        ...
        try {
            ...
            if (!rtti && type_id != 0) {
                rtti = const_cast<debug::Rtti*>(
                    current_image->rttidata()->typeFromTypeId(type_id));
                if (!rtti) {
                    return json;
                }
            }
    ...
    variable_s
        display_variable(sp::Symbol* sym, uint32_t index[], int idxlevel,
            bool noarray = false)
    {
        nlohmann::json json;
        variable_s var;
    ...
    int
        set_symbolvalue(const sp::Symbol* sym, int index, cell_t value)
    {
        cell_t* vptr;
        cell_t base = sym->addr();
        if (sym->vclass() & DISP_MASK)
    ...
    bool
        SetSymbolString(const sp::Symbol* sym, char* str)
    {
        assert(sym->ident() == sp::IDENT_ARRAY
            || sym->ident() == sp::IDENT_REFARRAY);
        assert(sym->dimcount() == 1);
