...
SmxV1Image::validateDebugVariables(const smx_rtti_table_header* rtti_table)
{
  for (uint32_t i = 0; i < rtti_table->row_count; i++) {
    ...
    if (!validateSymbolAddress(debug_var->address, debug_var->vclass, nullptr))
      return false;
...
bool
SmxV1Image::validateSymbolAddress(int32_t address, uint8_t vclass, const sp::debug::Rtti* rtti_type)
{
  switch (vclass) {
